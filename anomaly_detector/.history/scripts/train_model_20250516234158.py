...
# Read both tables
sat_df = pd.read_sql_query("SELECT * FROM satellite_readings", conn)
gps_df = pd.read_sql_query("SELECT * FROM gps_log", conn)
conn.close()

print("Total satellite records loaded:", len(sat_df))
print("Total GPS log records loaded:", len(gps_df))

# Merge on timestamp (you might consider rounding seconds or parsing to datetime if needed)
df = pd.merge(sat_df, gps_df, on="timestamp", how="left")

# Optional: convert timestamp to datetime object if needed for sorting
