...
def train_model_periodically():
    while True:
        subprocess.run([python_exec, '/home/isarasb/gps_anomaly/scripts/train_model.py'])
        time.sleep(3600)

def cleanup_db_periodically():
    while True:
        subprocess.run([python_exec, '/home/isarasb/gps_anomaly/scripts/start_logging.sh'])
        time.sleep(300)
