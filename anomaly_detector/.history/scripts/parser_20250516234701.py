            ...
            try:
                ...
                if isinstance(msg, pynmea2.types.talker.GGA):
                    ...
                    for i in range(0, len(sat_data), 4):
                        ...
                        try:
                            prn = int(sat_data[i])
                            elevation = int(sat_data[i + 1]) if sat_data[i + 1] != '' else None
                            azimuth = int(sat_data[i + 2]) if sat_data[i + 2] != '' else None
                            snr = float(sat_data[i + 3]) if sat_data[i + 3] != '' else None
                            if prn and snr and elevation and azimuth is not None:
                                cursor.execute("""
