...
# Convert 'snr' column to numeric, coercing errors into NaN
...
for prn in df['prn'].unique():
    ...
    prn_df['snr_diff'] = prn_df['snr'].diff()  # Calculate difference in SNR
    drops = prn_df[prn_df['snr_diff'] < -5]  # Threshold for significant drop
    if not drops.empty:
